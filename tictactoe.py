...
def minimax(board):
    ...
    if player(board) == X:
        for action in actions(board):
            if utility(result(board, action)) == minvalue(board):
                return action
    ...
    if player(board) == O:
        for action in actions(board):
            if utility(result(board, action)) == maxvalue(board):
                return action
        ...
        """""
        for action in actions(board):
            board_1 = result(board, action)  # all board possibilities for O
            #print(board_1)
            i = 0
            if winner(board_1) == O:
                return action
            for action_1 in actions(board_1):  # all board possibilities 1 move ahead for X
                board_2 = result(board_1, action_1)
                if winner(board_2) == X:
                    return action_1
        for action in actions(board):
            board_1 = result(board, action)
            for action_1 in actions(board_1):  # all board possibilities 1 move ahead for X
                board_2 = result(board_1, action_1)
                for action_2 in actions(board_2):  # we looking for board_5 (act. num 4 tho)
                    board_3 = result(board_2, action_2)
                    for action_3 in actions(board_3):
                        board_4 = result(board_3, action_3)
                        for action_4 in actions(board_4):
                            board_5 = result(board_4, action_4)  #maybe look for all combos of board but -1 for action that has a loss
                            for action_5 in actions(board_5):
                                board_6 = result(board_5, action_5)
                                if winner(board_6) == X:
                                    i += 1
                                    if i > 1300:
                                        print("hello")
            ...
            #for action_2 in actions(board_2):
                #board_3 = result(board_2, action_2)
                #if winner(board_3) == O:
                    #return action_2
        #print("random")
